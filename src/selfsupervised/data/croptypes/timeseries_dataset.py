...
class AugmentationSampling():
    ...
    def __init__(self, data, years) -> None:
        ...
        self.mu = np.zeros((self.no_years, len(self.types), len(self.channels),
                            self.time_steps))
        self.std = np.zeros((self.no_years, len(self.types),
                             len(self.channels), self.time_steps))
        print('I found that:')
        for n in range(self.no_years):
            for m in range(6):
                print('Year:', self.years[n], ' has a total of ',
                      len(data[str(self.years[n])][m][0]), 'samples for type ',
                      m)
            print('------------------------')
...
class TSAugmented(Dataset):
    ...
    def __init__(self,
        ...
        for n in range(tot_samples):
            for c in range(len(channel_idx)):
                data_dict[str(
                    data2[n * self.time_steps,
                          20])][data2[n * self.time_steps, 3]][c].append(
                              data2[n * self.time_steps +
                                    time_idx[0]:n * self.time_steps +
                                    time_idx[1], channel_idx[c]])
    ...
    def __getitem__(self, idx):
        ...
        # : Augmentation 1
        year = int(np.rint(
            self.n_years * np.random.rand())) - 1  # 0: 2016, 1: 2017, 2: 2018
        ...
        return (aug_x1, aug_x2
                ), torchx, torchy  # , torch.tensor(field_id, dtype=torch.long)
...
class CropInvarianceAug2(Dataset):
    ...
    def get_X1_X2(self, data, features):
        ...
        X1 = data[data.id == field_id1][features].to_numpy()
        X2 = data[data.id == field_id2][features].to_numpy()
        X1 = Drift(max_drift=0.1, n_drift_points=2).augment(
            X1)  # OwnAugmentation.constant_noise(X1, 7000)
        X2 = Drift(max_drift=0.1, n_drift_points=2).augment(
            X2)  # OwnAugmentation.constant_noise(X2, 7000)
...
class DriftNoiseAug(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (self.x2torch(aug_x1), self.x2torch(aug_x2)
                ), torchx, torchy  # , torch.tensor(field_id, dtype=torch.long)
