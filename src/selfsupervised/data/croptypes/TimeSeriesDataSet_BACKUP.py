...
class AugmentationSampling():
    ...
    def __init__(self, data) -> None:
        ...
        self.mu = torch.zeros(
            (self.years, self.types, self.channels, self.time_steps))
            ...
            for f in range(self.types):
                for c in range(self.channels):
                    for t in range(self.time_steps):
                        self.mu[y, f, c, t] = torch.mean(
                            torch.tensor(data[y, f, c, t, :]))
                        self.std[y, f, c, t] = torch.std(
                            torch.tensor(data[y, f, c, t, :]))
        ...
        for n in range(n_samples):
            for c in range(self.channels):
                for t in range(self.time_steps):
                    ...
                    samples[n, c,
                            t] = torch.normal(mean=self.mu[year, type, c, t],
                                              std=self.std[year, type, c, t])
        return samples
...
class TSAugmented(Dataset):
    ...
    def __init__(self,
        ...
        data_sorted = np.zeros(
            (n_years, n_features, n_channels, n_tsteps, n_samples))
                    ...
                    if (str(temp_data[n, -1]) == str(data.Year.unique()[y])):
                        if (temp_data[n, 3] == m):
                            if (cnt == 14):
                                fcnt += 1
                                cnt = 0
                            data_sorted[y, m, :n_channels, cnt,
                                        fcnt] = temp_data[n, 4:17]
                            cnt += 1
