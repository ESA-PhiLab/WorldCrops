...
class SimSiam_UNet_Encoder2(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.labels:
            ...
            return {'loss':loss, 'y_true': labels.detach(), 'embedding': embedding.detach()}
    ...
    def training_epoch_end(self, outputs):
        ...
        if self.labels:
            ...
            #log every 10 epochs
            if not self.current_epoch % 10:
                self.logger.experiment.add_embedding( torch.cat(embedding_list), global_step=self.current_epoch, tag = 'pretraining embedding') 
...
for filename in sorted(glob.glob(path_to_data + 'PNG_train/S1*.png')):
   ...
   filename3=path_to_data + 'Mask_train/NaN_mask_'+ name_part1 + name_part2 + '.png'
...
for filename in sorted(glob.glob(path_to_data + 'PNG_unlabeled/S1*.png')):
   ...
   # load corresponding mask (no satellite coverage)
   filename3=path_to_data + 'Mask_unlabeled/NaN_mask_'+ name_part1 + name_part2 + '.png'
   mask = Image.open(filename3)
...
dataset_train= lightly.data.LightlyDataset(
    input_dir=path_to_data + 'PNG_train/',
    #transform=test_transforms
)
dataset_unlabeled= lightly.data.LightlyDataset(
    input_dir=path_to_data + 'PNG_unlabeled/',
    #transform=test_transforms
)
...
tensor_x = torch.Tensor(x_train[:,np.newaxis, :, :]) # transform to torch tensor
...
model = SimSiam_UNet_Encoder2(_encoder, num_ftrs, proj_hidden_dim, pred_hidden_dim, out_dim)

#tb_logger = pl_loggers.TensorBoardLogger(save_dir="../../logs/")
trainer = pl.Trainer(gpus=cfg["pretraining"]['gpus'], deterministic=True, max_epochs = cfg["pretraining"]['epochs'])
