...
torch.save(backbone, "../model/pretrained_c2/pretraining1.ckpt")
#%%
#fit 
...
torch.save(backbone2, "../model/pretrained_c2/pretraining2.ckpt")
...
torch.save(backbone3, "../model/pretrained_c2/pretraining3.ckpt")
...
torch.save(backbone4, "../model/pretrained_c2/pretraining4.ckpt")
#%%
...
transfer_model = Attention_Transfer(input_dim=input_dim, num_classes = 6, d_model=num_ftrs, backbone = backbone_copy1, head=head, batch_size = batch_size, finetune=False, lr=lr)
if IARAI:
...
#use pretrained backbone and finetune 
transfer_model2 = Attention_Transfer(input_dim=input_dim, num_classes = 6, d_model=num_ftrs, backbone = backbone_copy2, head=head2, batch_size = batch_size, finetune=False, lr=lr)
if IARAI:
    trainer = pl.Trainer(gpus=no_gpus, strategy='ddp', deterministic=True, max_epochs = _epochs_fine)
else:
    trainer = pl.Trainer(deterministic=True, max_epochs= _epochs)
...
transfer_model3 = Attention_Transfer(input_dim=input_dim, num_classes = 6, d_model=num_ftrs, backbone = backbone_copy3, head=head3, batch_size = batch_size, finetune=False, lr=lr)
...
transfer_model4 = Attention_Transfer(input_dim=input_dim, num_classes = 6, d_model=num_ftrs, backbone = backbone_copy4, head=head4, batch_size = batch_size, finetune=False, lr=lr)
