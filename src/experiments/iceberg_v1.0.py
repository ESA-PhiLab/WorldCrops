...
'''
#######################################################################
Please note: The script only works with iceberg data that is not published here.
#######################################################################
'''
...
from selfsupervised.model.lightning.simsiam_unet import SimSiamUNet
from selfsupervised.model.lightning.unet_transfer import UNetTransfer
from selfsupervised.model.torchnn.unet import ResUnetEncoder
...
# train
tensor_x = torch.Tensor(np.repeat(x_train[:, np.newaxis, :, :], 3,
...
# validate
val_x = torch.Tensor(np.repeat(x_val[:, np.newaxis, :, :], 3,
                               axis=1))  # transform to torch tensor
...
# test
test_x = torch.Tensor(np.repeat(x_test[:, np.newaxis, :, :], 3,
                                axis=1))  # transform to torch tensor
...
_encoder = ResUnetEncoder(channel=channels,
                                    filters=filters,
                                    dropout=dropout)
model = SimSiamUNet(_encoder, num_ftrs, proj_hidden_dim,
...
model_finetune = UNetTransfer(backbone=backbone_pretrain)
