...
class Custom1617(Dataset):
    ...
    def __init__(self, data, feature_list = [], target_col = 'NC', field_id = 'id', time_steps = 14, size = 0):
        self.df = data
        self.target_col = target_col
        self.feature_list = feature_list
        self.time_steps = time_steps
...
features =['B3_mean','B4_mean','B5_mean','B6_mean','B7_mean','B8_mean','B8A_mean']
train = TSDataSet(bavaria_data, features, 'Year', field_id = 'id',  time_steps = 11)
train_dl = DataLoader(train, batch_size=batch_size_pre, shuffle=True, drop_last=False, num_workers=2)
...
model4 = selfsupervised.model.Attention_LM(input_dim=7, num_classes = cfg["transformer"]['num_classes'], n_head=cfg["transformer"]['n_head'], nlayers=cfg["transformer"]['nlayers'], batch_size = batch_size_fine, lr=lr_pre)
...
# %%
transformer = selfsupervised.model.Attention(input_dim= 7, num_classes = cfg["transformer"]['num_classes'], n_head=cfg["transformer"]['n_head'], nlayers=cfg["transformer"]['nlayers'])
...
trainer = pl.Trainer(gpus=_gpu, deterministic=True, max_epochs = 300, logger=tb_logger)
features =['B3_mean','B4_mean','B5_mean','B6_mean','B7_mean','B8_mean','B8A_mean']
train2 = Custom1617(bavaria_data, features, 'Year', field_id = 'id',  time_steps = 11, size=5000)
train_dl2 = DataLoader(train2, batch_size=batch_size_pre, shuffle=True, drop_last=False, num_workers=2)
...
trainer = pl.Trainer(gpus=_gpu, deterministic=True, max_epochs = 300, logger=tb_logger)
features =['B3_mean','B4_mean','B5_mean','B6_mean','B7_mean','B8_mean','B8A_mean']
train3 = Custom1718(bavaria_data, features, 'Year', field_id = 'id',  time_steps = 11, size=5000)
train_dl3 = DataLoader(train3, batch_size=batch_size_pre, shuffle=True, drop_last=False, num_workers=2)
#fit the first time with one augmentation
...
model4 = selfsupervised.model.Attention_Transfer(lr = lr_pre, input_dim = 7, num_classes = 3,d_model = 64, backbone=backbone, head=nn.Sequential(*list(transformer.children())[-1]), batch_size  = batch_size_fine, finetune= False)
trainer.fit(model4, train_dl)
