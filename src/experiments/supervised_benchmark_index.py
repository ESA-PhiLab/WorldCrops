...
lr = 0.0016612
#input dim is 1 because we use only one index
input_dim = 1
...
dm_bavaria = IndexDataModule(data_dir = '../../data/cropdata/Bavaria/sentinel-2/Training_bavaria.xlsx', batch_size = batch_size, num_workers = num_workers)
dm_bavaria2 = Index1617DataModule(data_dir = '../../data/cropdata/Bavaria/sentinel-2/Training_bavaria.xlsx', batch_size = batch_size, num_workers = num_workers)
dm_bavaria3 = Index1percentDataModule(data_dir = '../../data/cropdata/Bavaria/sentinel-2/Training_bavaria.xlsx', batch_size = batch_size, num_workers = num_workers)

# %%
# Plots the optimal learning rate

model = Attention_LM(input_dim = input_dim, num_classes = 6, n_head=4, nlayers=3, batch_size = batch_size, lr=lr, seed=42)
...
model2 = Attention_LM(input_dim = input_dim, num_classes = 6, n_head=4, nlayers=3, batch_size = batch_size, lr=lr)
...
# %%
trainer = pl.Trainer( gpus=1 if str(device).startswith("cuda") else 0, deterministic=True, max_epochs= _epochs)
model3 = Attention_LM(input_dim = input_dim, num_classes = 6, n_head=4, nlayers=3, batch_size = batch_size, lr=lr)
