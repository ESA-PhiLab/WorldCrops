...
class BavariaDataModule2(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.experiment == 'Experiment1':
            ...
            ts_train = TSDataSet(train, self.feature_list, 'NC', field_id = 'id',  time_steps = 14)
            ts_test = TSDataSet(test, self.feature_list, 'NC', field_id = 'id',  time_steps = 14)
...
class BavariaDataModule3(pl.LightningDataModule):
    def __init__(self, data_dir: str = "./", batch_size = 32, num_workers = 2, experiment='Experiment1'):
        super().__init__()
        self.data_dir = data_dir
        self.transform = None
...
class AugmentationExperiments2(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.experiment == 'Experiment1':
            train ,test = self.experiment1()
            ts_data = CropInvarianceAug2(train, self.feature_list, size=10000)

        elif self.experiment == 'Experiment2':
            train ,test = self.experiment2()
            ts_data = TSAugmented(train, factor=8, feature_list=self.feature_list)

        elif self.experiment == 'Experiment3':
            train, _ = self.experiment3()
            ts_data = CropInvarianceAug2(train, self.feature_list, size=10000, time_steps = 14)
            ...
            train, test = self.experiment3()
            ts_data = DriftNoiseAug(train, factor=8, feature_list=self.feature_list, time_steps = 14)

            a = train.mean()[self.feature_list].to_numpy()
            b = test.mean()[self.feature_list].to_numpy()
            diff = b-a
            #ts_data = Shift_TS(train, factor=8, diff = diff, feature_list = self.feature_list, time_steps = 14)
