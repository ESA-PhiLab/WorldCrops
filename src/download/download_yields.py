...
from sentinelhub import (CRS, BBox, CustomUrlParam, DataCollection, MimeType,
                         WmsRequest, bbox_to_dimensions)
...
PATH2YIELDS = '../../data/cropdata/Bavaria/yields/yields2018.csv'
# %%
yields = pd.read_csv(PATH2YIELDS,
                     sep=",",
                     encoding="ISO-8859-1",
                     engine='python')
yields = yields[[
...
f, ((ax1, ax2, ax3), (ax4, ax5, ax6), (ax7, ax8, ax9), (ax10, ax11, ax12),
    ...
    (ax19, ax20, ax21)) = plt.subplots(7, 3, figsize=(20, 40))
ax_list = [
...
gerste = [
    'Muehlacker', 'Grafenfeld', 'Krohberg', 'Radarstation', 'Sieblerfeld',
    'Striegelfeld', 'Viehhausen1', 'Viehhausen11', 'Viehhausen3', 'Viehhausen5'
]
...
for i in range(len(names)):
    ...
    ax_list[i].scatter(y=field.Latitude,
                       x=field.Longitude,
                       alpha=1,
...
# %%
# read test area
area = gpd.read_file("../../data/cropdata/Bavaria/Test_area.shp")
...
request = WmsRequest(data_collection=DataCollection.SENTINEL2_L1C,
...
len(data[:])
# %%
plot_image(data[8][:, :, [5, 4, 3]], 2.5, vmax=0.4)
# %%
# download all data
